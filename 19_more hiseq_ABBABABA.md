# ABBABABA with only homozygotes

I modifed the previous script into this one, which requires all sites in the ABBABABA test to be homozygous.  The motivation is to minimize the effect of genotype errors (Performs_ABBA_BABA_on_populations_only_homozygotes.pl):
	...
	if($temp[0] eq '#CHROM'){
		...
		if(($temp[0] ne "chrX")&&($temp[0] ne "chrY")&&($temp[0] ne "chrM")){
			...
			if((uc $temp[$whotoinclude[0]-1] eq "A")||(uc $temp[$whotoinclude[0]-1] eq "C")||(uc $temp[$whotoinclude[0]-1] eq "T")||(uc $temp[$whotoinclude[0]-1] eq "G")){
				...
				$ancestral=0;
				for ($y = 0 ; $y <= $#H3; $y++ ) {
					if(($temp[$H3[$y]+$whotoinclude[1]-2] eq 'A/A')||($temp[$H3[$y]+$whotoinclude[1]-2] eq 'C/C')||($temp[$H3[$y]+$whotoinclude[1]-2] eq 'G/G')
						||($temp[$H3[$y]+$whotoinclude[1]-2] eq 'T/T')){
				...
				$ancestral=0;
				for ($y = 0 ; $y <= $#H1; $y++ ) {
					if(($temp[$H1[$y]+$whotoinclude[1]-2] eq 'A/A')||($temp[$H1[$y]+$whotoinclude[1]-2] eq 'C/C') || ($temp[$H1[$y]+$whotoinclude[1]-2] eq 'G/G')
						|| ($temp[$H1[$y]+$whotoinclude[1]-2] eq 'T/T')){
				...
				$ancestral=0;
				for ($y = 0 ; $y <= $#H2; $y++ ) {
					if(($temp[$H2[$y]+$whotoinclude[1]-2] eq 'A/A') || ($temp[$H2[$y]+$whotoinclude[1]-2] eq 'C/C') || ($temp[$H2[$y]+$whotoinclude[1]-2] eq 'T/T')
						|| ($temp[$H2[$y]+$whotoinclude[1]-2] eq 'G/G')){
				...
				if(($derived+$ancestral)>0){
					...
					@H2allelez=();
					for ($y = 0 ; $y <= $#H3; $y++ ) {
						if(($temp[$H3[$y]+$whotoinclude[1]-2] ne './.')&&($temp[$H3[$y]+$whotoinclude[1]-2] ne 'N/N')&&
							($temp[$H3[$y]+$whotoinclude[1]-2] ne '*/*')&&($temp[$H3[$y]+$whotoinclude[1]-2] ne '*/.')&&
							(length($temp[$H3[$y]+$whotoinclude[1]-2]) == 3)){
							for ($w = 0 ; $w <= $#H2; $w++ ) {
								if(($temp[$H2[$w]+$whotoinclude[1]-2] ne './.')&&($temp[$H2[$w]+$whotoinclude[1]-2] ne 'N/N')&&
					...
					for ($y = 0 ; $y <= $#H3; $y++ ) {
						if(($temp[$H3[$y]+$whotoinclude[1]-2] ne './.')&&($temp[$H3[$y]+$whotoinclude[1]-2] ne 'N/N')&&
							...
							(length($temp[$H3[$y]+$whotoinclude[1]-2]) == 3)){
							for ($w = 0 ; $w <= $#H1; $w++ ) {
								if(($temp[$H1[$w]+$whotoinclude[1]-2] ne './.')&&($temp[$H1[$w]+$whotoinclude[1]-2] ne 'N/N')&&
					...
					for ($y = 0 ; $y < $#H2; $y++ ) {
						for ($w = ($y+1) ; $w <= $#H2; $w++ ) {
							if(($temp[$H2[$y]+$whotoinclude[1]-2] ne './.')&&($temp[$H2[$y]+$whotoinclude[1]-2] ne 'N/N')&&
					...
					for ($y = 0 ; $y < $#H1; $y++ ) {
						for ($w = ($y+1) ; $w <= $#H1; $w++ ) {
							if(($temp[$H1[$y]+$whotoinclude[1]-2] ne './.')&&($temp[$H1[$y]+$whotoinclude[1]-2] ne 'N/N')&&
				...
				# For ABBABABA, only consider sites for which there are data for H1, H2, and H3
...
```R
# read the data
data<-read.table("concat_homoz.stats",header=TRUE)

# make a column for weights
data$ABBA_BABA_sites<-data$BABA+data$ABBA

# weighted histogram with ggplot
library(ggplot2)
pdf("ABBABABA_weightedhistogram.pdf",w=8, h=4, version="1.4", bg="transparent")
ggplot(data, aes(data$f_dm, weight = data$ABBA_BABA_sites)) + geom_histogram(bins = 35) +
  theme_bw() + theme(panel.grid.minor=element_blank(),panel.grid.major=element_blank())
dev.off()
# unweighted histogram with ggplot
pdf("ABBABABA_unweightedhistogram.pdf",w=8, h=4, version="1.4", bg="transparent")
ggplot(data, aes(data$f_dm)) + geom_histogram(bins = 35) +
  theme_bw() + theme(panel.grid.minor=element_blank(),panel.grid.major=element_blank())
dev.off()

unweighted_fDM<-mean(data$f_dm, na.rm = TRUE)
cat("The genomewide unweighted fDM is :",unweighted_fDM)
weighted.mean(data$f_dm, data$ABBA_BABA_sites, na.rm = TRUE)

# calculate standard error
library(resample)
# calculate the mean number of ABBA_BABA sites
mean_number_of_ABBA_BABA<-mean(data$ABBA_BABA_sites)
# make a colum with the f_dm values weighted by the number sites divided by the mean of ABBA_BABA sites
...
sterr_f_dm<-sqrt(var_replicates/(length(which(!is.na(data$f_dm)))-1)) 

# Report the weighted meal for fDM:
genome_wide_fdm<-mean(data$weighted_f_dm, na.rm=TRUE)
genome_wide_fdm
cat("The genomewide weighted fDM is :",genome_wide_fdm)
# Report the stdev
cat("The standard deviation is :",sterr_f_dm)
# Report the sterr
stdev_f_dm<-sqrt(var_replicates) 
cat("The standard deviation is :",stdev_f_dm)
# Report the Zstat from sterr
Z_stat_from_sterr<-mean(data$weighted_f_dm, na.rm=TRUE)/sterr_f_dm
cat("The Zstat from sterr is :",Z_stat_from_sterr)
# Report the Zstat from stdev
Z_stat_from_stdev<-mean(data$weighted_f_dm, na.rm=TRUE)/stdev_f_dm
cat("The Zstat from stdev is :",Z_stat_from_stdev)
